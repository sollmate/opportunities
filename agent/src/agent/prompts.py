...
3. READ STATE FIRST. At the start of a run, read `/master_data.json` and
   `/uploads/_index.json` (the catalog of every user upload). Open each
   upload's `virtual_path` whose columns are relevant to the case. There is no
   canonical `/ledger.json` — uploads carry whatever schema the user provided.
...
# Operating procedure
1. Read /master_data.json and /uploads/_index.json; open each upload by its
   `virtual_path`. Confirm the SKR variant if any upload is a DATEV ledger.
...
- READ FILES FULLY. The built-in `read_file` returns only a slice of a file. NEVER stop
  ...
  characters / lines) until you hit EOF. This applies to /master_data.json,
  /uploads/*, and every SKILL.md.
